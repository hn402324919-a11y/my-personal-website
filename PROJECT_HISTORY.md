...
- 最新待上线修改：移动端项目卡片高度与「营销 / 运营」项目卡片媒体区域下方留白优化。
- 本次推送后，Vercel 会从 `main` 自动部署项目展示区移动端视觉调整：收紧移动端项目卡片整体高度、标题区域边距与标题/副标题间距，并让「营销 / 运营」封面内部三张图保持当前尺寸比例和排列方式的同时下移媒体组，减少图片结束后到标题文案之间的无效留白。
