...
- 调整 `app/globals.css` 中 `min-width:1101px` 的项目网格行高关系。
- 使用第 1 行和第 3 行的响应式平均值，让左上跨行项目卡片与右下跨行营销项目卡片高度一致，同时保持项目模块总高度不变。
- 左下绿色圆形视觉卡片和「B 端 / 网页」卡片随第 3 行同步变矮，保留原有列宽、间距、圆角和 grid 布局逻辑。
- 优化绿色 `DESIGN POSITION / 09Y` 模块桌面端主标题排版：
  - 标题增加响应式宽度上限；
  - 桌面端隐藏原强制换行；
  - 英文文案、字体、字号、字重、行高风格和移动端样式保持不变。
- 功能提交：`9461395 fix: tune desktop project layout`。
- 验证：`pnpm run lint`、`pnpm run build`、`pnpm test` 均通过，保留既有图片优化 warning、chunk size warning 和 vinext route classification 提示。
...
- 修复桌面端项目卡片分类标签位置：`体验 / 界面`、`营销 / 运营`、`B 端 / 网页` 从图片区域右上角移动到左上角，移动端保持右上角现状。
- 根据本地预览注释进一步精确锚定项目卡片分类标签：桌面端 `.card-top` 显式取消右侧锚点，确保文案距卡片左侧保持 `24px`。
- 优化首次进入网站的 loading 页面：绿色光晕改为不被顶层容器裁切的柔和扩散层，`CHENYNII` 与 `PORTFOLIO · 2017—2026` 保持左侧锚点和左对齐。
- 修复 loading 优化后的回归：恢复底部进度条可见宽度，并将 loading 文字组改为 flex 纵向布局，让主标题和副标题共享同一左侧基准线。
- 恢复满意版首页开屏动画作为当前稳定基准，同时保留项目分类标签左上角定位；移动端项目标签现在与 PC 端一致锚定左上角。
- 优化移动端 opening sequence 开屏文字排版：收紧 `PORTFOLIO · 2017—2026` 字距，保持年份单行显示，并增加标题、年份和底部横线之间的呼吸感；桌面端样式不变。
- 对齐移动端顶部导航左右安全距离：`.nav` 与 `.nav.is-fixed` 在移动端使用 `var(--shell)`，与个人信息卡片和正文内容保持一致；导航圆角、背景、按钮尺寸和桌面端样式不变。
- 修复移动端滚动到第二屏后左上角露出绿色“跳到主要内容”按钮的问题：`.skip` 默认改为视觉裁剪隐藏，触屏普通 focus 状态保持隐藏，键盘可见焦点仍可显示并跳转到 `#content`。
