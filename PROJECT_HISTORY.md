...
- 最新待上线修改：桌面端 Project Cards 比例与「营销 / 运营」封面完整展示优化。
- 本次推送后，Vercel 会从 `main` 自动部署项目展示区视觉调整：桌面端 Project Cards 更紧凑且保持非等高 Masonry / Grid 节奏；「营销 / 运营」封面内部三张图按真实比例 `contain` 完整展示，并整体下移以收紧与底部标题文案之间的空白。
