...
- 最新待上线修改：PROJECT 2「伊利 QQ 星亲子打卡」项目详情图片替换。
- 本次推送后，Vercel 会从 `main` 自动部署 PROJECT 2 图片资源修正：「伊利 QQ 星亲子打卡」从错误单图替换为新的 4 张纵向 WebP 分片；继续沿用 `CaseGallery` 渐进挂载逻辑，原有切片无缝拼接、gap、边框和圆角效果保持不变。
